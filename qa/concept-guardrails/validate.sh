...
SKILL_DIR="$(cd "$QA_DIR/../../skills/concept-guardrails" && pwd)"
...
check_skill_layout() {
  ...
  rg -q '^name: concept-guardrails$' "$SKILL_DIR/SKILL.md" || fail "frontmatter name mismatch"
  ! rg -q '^license:' "$SKILL_DIR/SKILL.md" || fail "SKILL.md must omit license to avoid ClawHub MIT-0 conflict"
  rg -q '\$concept-guardrails' "$SKILL_DIR/agents/openai.yaml" || fail "default prompt must name the skill"
...
printf 'OK: concept-guardrails validation passed\n'
