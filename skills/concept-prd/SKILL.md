---
name: concept-prd
description: Transcribes a confirmed Jackson concept model into PRD specs (central overall PRD plus wyx-compatible colocated CONCEPT.md and flow-grouped SYNCS.md). Use this skill whenever the user asks for a concept PRD, 规格文档, or mentions concept-prd (formerly jackson-concept-prd); do not invent model content.
---

# Jackson 概念 PRD（设计规格）

输入是已确认的概念模型（`concept-design` 的输出）。概念设计即设计规格：本技能只做转录与编排，不发明模型外的内容；发现模型缺口时回上游技能补模，不在文档里私自填补。文档交付后，代码落地用 `concept-implementation`。
...
- **共存优先**：模块目录已存在（回填或实现后）就把 `CONCEPT.md` 放进去；代码未就绪先集中暂存，落地时由 `concept-implementation` 迁移共存。
...
转录期全部 sync 单文件、自带完整协调图；实现期 syncs 层按概念分组拆包时，由 `concept-implementation` 把本文件按 flow 群随包拆分（flow 不拆散，协调图随包局部化）。
...
（sync 块，when / where / then 记法同 concept-design；
...
- SYNCS.md 转录期单文件、有 coordination graph、按 flow 分节（实现期拆包归 `concept-implementation` 管辖）；每个可失败动作有错误 sync 或记入排除。
