...
从指定子树（默认全仓）的规格派生 `ARCHITECTURE.md`：同步关系、产品依赖、数据流与覆盖情况。地图服务导航，不被 hooks 消费。无规格则报告并建议 `wyx:concept`，不生成空图。
...
3. 读取所有参与建图的内容：概念名称/purpose、sync 的 app/include/when/where/then、旧版协调图/边界段、管道来源/输出/data boundary，以及 PRD 产品依赖。
4. 默认重新派生并比较结果；相同则不改文件。mtime 或仅比较旧边界段不足以跳过：删除、重命名、暂存文件、sync 条件变化都可能改变地图。只有保存过完整来源路径集合与内容指纹时，才能据此安全跳过。
...
Jackson 方言以 `sync` 块为权威。每条 sync 一个规则节点；全部 when 动作连入，全部 then 动作连出；where 查询另用虚线标为“读取/绑定”，不画成触发动作。多 when 是共同匹配，不是任一来源即可触发；保留合取条件和参数关联。

旧版从 `## coordination graph` 与 `## sync:` 条目提取；两者冲突时报告，不能用优先级掩盖矛盾。级联或环按实际画出，并标注终止条件；环本身不是缺陷。

### 产品依赖

只用总体 PRD 明确的 extrinsic 依赖：`A → B` 表示纳入 A 的产品需要 B。与设计/审计方向一致；**不能从 sync 的 A→B 推导产品依赖**。未声明的依赖标未定义。

旧版 `dependencies` 可能混有模块调用、数据库、API、环境配置；按原意分别标注，不冒充产品依赖。外部依赖只有确为外部系统时才建 external 节点。
