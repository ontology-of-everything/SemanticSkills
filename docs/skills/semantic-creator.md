...
**Version:** 0.4.0 · Changelog: [qa/semantic-creator/CHANGELOG.md](../../qa/semantic-creator/CHANGELOG.md)
...
| Design review | HTML 评审报告（model JSON 注入壳模板）：证据 + 置信度（confirmed/inferred/assumed）+ 批注 JSON 回传（clipboard / 离线降级）→ `amendments.md` 迭代；未批准硬停不 Emit |
...
Phase 1 · Ingest ── 操作清单 → fact | dimension-lookup（退出条件达成才前进）
     │
     ▼
Phase 2 · Review ── 访谈收集（证据+置信度）→ HTML 评审报告（模板+数据）→ 硬停等批注 → amendments.md → 批准
     │
     ▼
...
| Critical Rules | Evidence-only；grain first；one blocking ask；layer split；stable names；review gate（未批准硬停） |
...
├── SKILL.md
├── assets/
│   └── review-template.html   # Phase 2 评审报告壳（内联渲染器 + vendored petite-vue，离线无 CDN）
└── references/
