...
Note on formatting: in the original raw markdown, the indented state/action/query declaration blocks use single leading spaces; they have been indented here with tabs for readable rendering, with content unchanged. One formatting inconsistency in the original (the indentation of the *Labeling* example's `a name String` line) has been aligned for rendering; no wording was altered.
